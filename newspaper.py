...
SECTION_MAP = [
    ...
    # 2026-09-03. CAREER and LEARNING moved off this page to
    # career.askakshay.com, which is built for the job search rather than
    # bolted onto the Life page. Removing the rows is the whole change: `secs`
    # is derived from them, so the `{% if 'x' in secs %}` guards skip the
    # markup and the nav groups go with it. The generators (jobs.py,
    # daily_learning.py, book_deep.py, podcasts.py) are left in the tree but
    # are no longer called from here — see the call sites below.

    # 2026-09-03: PRACTICE, MIND and DRILLS followed CAREER and LEARNING to
    # career.askakshay.com. There is no /desk page any more — SECTION_MAP is
    # main-page-only. See the note above the template's </main>.
]
...
WATCHLIST = [
    ...
    # TATAMOTORS.NS 404s since the demerger — see signals/universe.py.
    "ADANIENT.NS", "TMCV.NS", "TMPV.NS", "SUNPHARMA.NS", "IRCTC.NS", "TATAPOWER.NS",
...
<!-- ══════════ THE LIFE PAGE MOVED 2026-09-03 ══════════
     Every /desk section now lives on career.askakshay.com:
       Career   — Finance Careers, CFO Track
       Learning — Daily Brief, Smart Reads, Book, Podcasts
       Practice — Language, Father
       Mind     — Wisdom, The Mind, The Way, The Review, The Desk
       Drills   — Chess, Mind Gym
     All fifteen ids are recorded in test_page_structure.RETIRED.

     THE GENERATORS STILL RUN AND MUST KEEP RUNNING. generate.py writes every
     one of these payloads into docs/today.json under "desk", and
     career.askakshay.com fetches that file client-side. Stop calling
     daily_learning.py, podcasts.py, book_deep.py or the lichess fetchers and
     the new site goes blank — the data has one producer and it is still here.
-->
